...
if [ $? -eq 0 ]; then
    ...
    echo "2. System auto-initializes on first deposit"
    echo "3. Deposit APT: aptos move run --function-id <ADDRESS>::tinypay::deposit --type-args 0x1::aptos_coin::AptosCoin --args u64:<AMOUNT> vector<u8>:[] --profile testnet"
    echo "4. Add USDT support (admin): aptos move run --function-id <ADDRESS>::tinypay::add_coin_support --type-args <USDT_TYPE> --profile testnet"
    echo "5. Deposit USDT: aptos move run --function-id <ADDRESS>::tinypay::deposit --type-args <USDT_TYPE> --args u64:<AMOUNT> vector<u8>:[] --profile testnet"
    echo "6. Complete payments using complete_payment function with appropriate coin type"
    echo ""
