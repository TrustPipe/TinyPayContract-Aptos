module tinypay::usdc {
    ...
    public fun get_metadata(): Object<Metadata> {
        let asset_address = object::create_object_address(&@tinypay, ASSET_SYMBOL);
    ...
    public entry fun set_pause(pauser: &signer, paused: bool) acquires State {
        ...
        let state = borrow_global_mut<State>(object::create_object_address(&@tinypay, ASSET_SYMBOL));
    ...
    fun assert_not_paused() acquires State {
        let state = borrow_global<State>(object::create_object_address(&@tinypay, ASSET_SYMBOL));
    ...
    // Public function for external tests to initialize the USDC FA
    #[test_only]
    public fun init_for_test(admin: &signer) {
        init_module(admin);
    }

    #[test(creator = @tinypay)]
    fun test_basic_flow(
        creator: &signer,
    ...
    #[test(creator = @tinypay, aaron = @0xface)]
