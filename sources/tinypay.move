...
module tinypay::tinypay {
    ...
    public fun init_system(admin: &signer) {
        ...
        let (resource_signer, signer_cap) =
            account::create_resource_account(admin, b"tinypay_vault");
    ...
    public entry fun merchant_precommit(
        ...
        let precommit = PreCommit { merchant: signer::address_of(merchant), expiry_time };
    ...
    public entry fun complete_payment(
        ...
        assert!(payer == precommit.merchant, E_INVALID_PRECOMMIT);
        ...
        if (user_account.max_tail_updates > 0) {
            assert!(
                user_account.tail_update_count < user_account.max_tail_updates,
                E_TAIL_UPDATES_LIMIT_EXCEEDED
            );
        };
    ...
    public fun get_user_limits(user_address: address): (u64, u64, u64) acquires UserAccount {
        ...
        let user_account = borrow_global<UserAccount>(user_address);
        (
            user_account.payment_limit,
            user_account.tail_update_count,
            user_account.max_tail_updates
        )
    }
